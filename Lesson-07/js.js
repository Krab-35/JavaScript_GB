...
const settings = {
  ...
  speed: 2,
  winFoodCount: 50,
  wallPointCount: 50, // количество препятствий
};
...
const map = {
  ...
  render(snakePointsArray, foodPoint, wallPointArray) {
    ...
    wallPointArray.forEach((pointWall) => {
      const wallCell = this.cells[`x${pointWall.x}_y${pointWall.y}`];
...
const snake = {
  ...
  score() { // вывод счета
    if (this.getBody().length - 1 != 0) {
      document.getElementById('score').remove();
    }
    const containerElement = document.createElement('div');
    containerElement.setAttribute('id', 'score');
    containerElement.classList.add('score');
    containerElement.innerHTML = 'Счет: ' + (this.getBody().length - 1);
...
const wall = {
  ...
  pushPointOfWall(element) {
    this.getBodyWall().push(element);
  },

  inWall(point) {
    return this.getBodyWall().some((wallPoint) => {
      return wallPoint.x === point.x && wallPoint.y === point.y;
    });
  },
...
const game = {
  ...
  reset() {
    ...
    this.addWalls(this.config.settings.wallPointCount); // создание произвольных координат стен
    this.render();
  },
  ...
  addWalls(pointCount) { // метод создания стен
    this.wall.resetWallPoints();
    for (let elem = 0; elem < pointCount; elem++) {
      this.wall.pushPointOfWall(this.getRandomFreeCoordinates());
    }
  },
  ...
  canMakeStep() { // проверка возможности сделать следующий шаг
    const nextHeadPoint = this.snake.getNextStepHeadPoint();
    return !this.snake.isOnPoint(nextHeadPoint) &&
      !this.wall.inWall(nextHeadPoint) &&
  ...
  render() {
    this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.wall.getBodyWall());
  },
};
